...
# All Z values are offsets from Z=0, which is wherever you manually parked
# the nozzle before the run.
...
WALL_ORIGIN_Z    = 5.0    # mm   Z of the bottom face of the very first row of bricks
                           #      relative to your manual Z=0 start position
...
def placement_nozzle_z(row: int) -> float:
    ...
    All Z values are offsets from Z=0 (the manually-parked start position).
    """
...
def generate_gcode(blocks, num_cols: int, num_rows: int) -> str:
    ...
    emit(
        ...
        f"; Wall Z     : {WALL_ORIGIN_Z:.1f} – {WALL_ORIGIN_Z + num_rows * BRICK_HEIGHT:.1f} mm  (offsets from Z=0)",
