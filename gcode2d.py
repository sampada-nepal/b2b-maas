...
DISPENSERS = {
    "RED": {
        "x": 24.0,   # mm  ← YELLOW + 3 studs (15 + 3×8), rightmost dispenser
        "y": 0.0,    # mm  ← calibrated via jog
        "z": 3.4,   # mm  ← tune until block grabs reliably (negative = below manual Z=0)
    },
    "YELLOW": {
        "x": 0.0,   # mm  ← calibrated via jog, leftmost dispenser
        "y": 0.0,    # mm  ← calibrated via jog
        "z": 3.4,   # mm  ← tune until block grabs reliably (negative = below manual Z=0)
    },
}
...
WALL_X           = 64.0   # mm   YELLOW + 8 studs (15 + 8×8 = 79) — stud-aligned
WALL_ORIGIN_Y    = 32.0   # mm   Y of column 0 — stud-aligned (4 × 8 mm)
WALL_ORIGIN_Z    = 0.0   # mm   Z of row 0 — 5 mm below manual Z=0 to engage studs
...
SAFE_Z           = 70.0   # mm   Z for all XY travel (clear of everything)
FEED_TRAVEL      = 20000   # mm/min  fast XY + Z travel (no brick)
FEED_CARRY       = 12000   # mm/min  travel speed while holding a brick
FEED_APPROACH    = 4000   # mm/min  slow descent before placement zone
FEED_PUSH        = 500    # mm/min  very slow final push onto studs
...
def generate_gcode(blocks, num_cols: int, num_rows: int) -> str:
    ...
    emit(
        ...
        "G92 X-11 Y-7 ; offset origin: true home = -11,-5 so X0 Y0 = our working origin",
        "G92 Z3.8                 ; declare current Z as Z=3.2 (height of one plate) (manually parked before run)",
