...
# ── LEGO wall origin ──────────────────────────────────────────────────────────
# The origin (Z=0 start position) is at the TOP-RIGHT of the wall, co-located
# with the block dispenser.  The wall builds LEFT (−X) and DOWN (−Z) from there.
#   col 0  = rightmost column  →  X = 0
#   col N  = leftmost  column  →  X = −N * BRICK_WIDTH
#   row 0  = top    row        →  Z = 0
#   row N  = bottom row        →  Z = −N * BRICK_HEIGHT
WALL_ORIGIN_X    = 0.0    # mm   X of column 0 (right edge, at origin)
WALL_ORIGIN_Y    = 150.0  # mm   Y of the wall (constant — wall is 1 brick deep)
WALL_ORIGIN_Z    = 0.0    # mm   Z of row 0 top face (at origin)
...
# ── Manual Z start position ───────────────────────────────────────────────────
# Z homing is SKIPPED (PINDA removed; LEGO baseplate covers the bed).
# Before running: manually jog the nozzle to a safe height above the baseplate.
# That position is declared as Z=0; all Z values above are offsets from it.
# Only X and Y are homed automatically (G28 X Y).
...
def brick_x(col: int) -> float:
    """Nozzle X for a given column (col 0 = rightmost, at X=0)."""
    return WALL_ORIGIN_X - col * BRICK_WIDTH
...
def generate_gcode(blocks, num_cols: int, num_rows: int) -> str:
    ...
    emit(
        ...
        f"; Wall X     : {WALL_ORIGIN_X:.1f} (right) → {WALL_ORIGIN_X - num_cols * BRICK_WIDTH:.1f} mm (left)",
    ...
    emit(
        ...
        "G28 X Y                ; home X and Y only — Z homing skipped (no PINDA, baseplate on bed)",
        "G92 Z0                 ; declare current nozzle position as Z=0 (manually parked before run)",
